    ...
    for (const tc of pageTokenCases) {
      it(`${tc.id}: ${tc.description}`, () => {
        if (tc.when === 'createTimeToPageToken') {
          ...
          const token = createTimeToPageToken(
            given.createTime!,
            given.exclusive ?? true,
          );

          if (tc.then.tokenStartsWith) {
            expect(token.startsWith(tc.then.tokenStartsWith)).toBe(true);
          }
          if (tc.then.tokenLength) {
            expect(token.length).toBe(tc.then.tokenLength);
          }
          if (tc.then.endsWithZero) {
            expect(token.endsWith('0')).toBe(true);
          }
