...
export class JulesMCPServer {
  ...
  private async handleQueryHelp(args: any) {
    const topic = (args?.topic as string) || 'examples';

    const helpContent = this.getQueryHelpContent(topic);
  ...
  private getQueryHelpContent(topic: string): string {
    switch (topic) {
      case 'where':
        return `## WHERE Clause
...
\`\`\``;
  ...
  private async handleValidateQuery(args: any) {
    ...
    // Return both structured result and formatted message
    return {
      content: [
        {
          type: 'text',
